---
title: "Documentos<br/>Reprodutíveis<br/>em R Markdown"
...
- as linguagens de programação *Python* e *SQL*
    ...
    - "Quais outras linguagens?" --> Rode: `names(knitr::knit_engines$get())`
- o sistema de preparação de documentos *LaTeX*
- o pacote *Shiny* para montar web apps e dashboards
- outras fontes de dados acessíveis via sistema de API ou raspáveis de forma 
  mais geral
...
- Fonte: 
  ...
  `r emo::ji("laughing")` \ 
  <font size=1>
  Não faça citações à Wikipedia no seu trabalho de faculdade, monografia, 
  dissertação ou tese!
  </font>
- Donald Knuth == inventor do TeX, que é um sistema tipográfico eletrônico e 
  base do LaTeX
- documento == descrição do documento + pedaços de texto + pedaços de código
- <font size=1>Não faça citações à Wikipedia: apesar de muitas páginas terem
...
- Fonte: 
  ...
  \ \ \  
  `r emo::ji("laughing")`
- "documentos" autocontidos: jupyter notebooks, git projects de R Markdown com 
  os microdados
...
- "In a DVCS (such as Git, Mercurial, Bazaar or Darcs), clients don’t just check 
  ...
  including its full history."
    ...
    - DVCS == distributed VCS == sistema de controle de versão distribuído
...
- um exemplo de SVC é o *git*, 
    - sistema de controle *distribuido* de versão
        - *diff* nos arquivos para individualizar as modificações
        - *commit* no seu repositório local das alterações
        - documentação das modificações feitas
...
- Enquanto o *Markdown* é uma *linguagem de marcação*, o *R Markdown* é um 
  formato de arquivo (cuja extensão geralmente é `.rmd` ou `.Rmd`) que contém 
  três tipos de conteúdo:
    - metadados sobre o documento, em formato [YAML](https://yaml.org/)
    - texto, em formato Markdown, que permite incluir 
        - hyperlink
        - imagem
        - tabela
...
Versão de quem tá com pressa do que é *API*:

- É um conjunto de 
    - um protocolo de troca de informação entre programas através da internet
    - programas que ficam rodando de acordo com esse protocolo, quer para 
      solicitar dados, quer para atender à solicitação


## Sistema de Interface de Programação de Aplicações *API*

Uma versão mais apressada ainda:

- O seu computador pede (com jeitinho) alguma informação pra outro computador e 
  ele faz o processamento que for necessário e te envia o que foi pedido
    - com jeitinho == seguindo a sintaxe da API


## Sistema de Interface de Programação de Aplicações *API*

Versão menos apressada:

- é uma forma de intergrar sistemas
- pode ser para outras finalidades, como prestação de serviços
- podem ser interna à máquina ou intranet
