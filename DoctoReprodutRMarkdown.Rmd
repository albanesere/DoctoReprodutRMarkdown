...
- permite rastrear a versão documento
    - sabe o "*putz, não podiam ter jogado fora aquele gráfico*" e o 
      "*é, ninguém tá achando a versão anterior da arquivo*"?
    - pois é, isso não acontece se você usar um sistema de controle de versão, 
...
Usaremos o JupiterWeb para gerar páginas dinâmicas via API no exemplo, mas 
agora sua URL será usada para ilustrar a estrutura da URL de uma requisição API
...
## O Exemplo | Vantagens adicionais

- se eu tivesse um vetor de códigos de curso, seria possível gerar uma sequência 
de tabelas adaptando um loop no código original
